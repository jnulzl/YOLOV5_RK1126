...
int main(int argc, char* argv[])
{
    ...
    {
        std::cout << "Usage:\n\t "
                  << argv[0] << " model_path input_size image_list save_root"
                  << std::endl;
        return -1;
    }
    ...
    int model_size;
    uint8_t* model_buf = load_model(weights_path.c_str(), &model_size);
    config_tmp.input_names = input_names;
    config_tmp.output_names = output_names;
    config_tmp.model_buffer = model_buf;
    config_tmp.model_size = model_size;
    ...
    std::string img_path;
    std::string img_save_root = std::string(argv[4]);
    long frame_id = 0;
    while (true)
    {
        cv::Mat frame;
