...
def yolov5_post_process(z, net_input_size, img_origin):
    '''
    ...
    if det is not None and len(det):
        ...
        input_height, input_width = net_input_size, net_input_size
        det[:, :4] = scale_coords((input_height, input_width), det[:, :4], img_origin.shape).round()
    return det
...
if __name__ == '__main__':
    ...
    rknn = RKNN()
    if len(sys.argv) < 4:
        print('Usage : \n\tpython %s rknn_model_path input_size img_list'%sys.argv[0])
        exit(0)
    
    input_size = int(sys.argv[2])
    ret = rknn.load_rknn(sys.argv[1])
    if ret != 0:
    ...
    with open(sys.argv[3], 'r') as fpR:
        ...
        for line in lines:
            ...
            det = yolov5_post_process(outputs, input_size, img_show)
            if det is None:
                continue
            boxes = det[:,0:4]
            scores = det[:,4]
